...
def run_metric_model_vari_convex(df_data):
...
#-------------------------------------------------------------------
# 6B. CONVEXITY / STARTING-POINT DIAGNOSTIC
#-------------------------------------------------------------------

    def total_waiting_time_item(i, mu_ij=None, var_ij=None, theta_ij=None, EBO_ij=None):
        """
        Total expected logistics waiting time for one SKU i across all bases.

        Following the waiting-time objective used in the thesis:
            TW_ij = lambda_ij * theta_ij * E_j + EBO_ij

        The depot itself is not included in the customer-facing objective.
        """
        if mu_ij is None or var_ij is None or theta_ij is None:
            mu_ij, var_ij, theta_ij = compute_mu_ij()
        if EBO_ij is None:
            EBO_ij = compute_EBO(mu_ij, var_ij)

        return sum(
            lambda_ij[(i, j)] * theta_ij[(i, j)] * E_j[j]
            + EBO_ij[(i, j)]
            for j in L
            if j != 0
        )
    ...
    def _find_discrete_convex_start(objective_values, tol=1e-10, min_tail_points=3):
        """
        Find the lowest stock level from which the discrete objective is convex.

        For a decreasing convex objective f(s), marginal improvements
            Delta(s) = f(s) - f(s+1)
        should be non-increasing as stock grows. Equivalently,
            f(s) - 2 f(s+1) + f(s+2) >= 0.

        We require this condition to hold for the remaining tested tail.
        """
        ...
        marginal_reductions = values[:-1] - values[1:]
        second_differences = values[:-2] - 2 * values[1:-1] + values[2:]

        # Need enough points after the proposed start to avoid declaring
        # convexity from only one final comparison.
        latest_start = max(0, len(second_differences) - min_tail_points + 1)

        for start in range(latest_start + 1):
            if np.all(second_differences[start:] >= -tol):
                return start, marginal_reductions.tolist(), second_differences.tolist()

        return None, marginal_reductions.tolist(), second_differences.tolist()


    def check_convexity_per_item(max_stock=20, tol=1e-10, min_tail_points=3):
        """
        Check discrete convexity of the waiting-time objective for every SKU.

        Procedure per item:
        1. Set that item's stocks to zero at all locations.
        2. Vary depot reorder point s_i0 from 0..max_stock and evaluate total
           waiting time across all bases.
        3. Identify the first depot stock from which marginal waiting-time
           reductions are non-increasing over the remainder of the tested range.
        4. Hold the depot at that identified starting value and vary each base
           stock S_ij separately from 0..max_stock.

        Other SKUs are irrelevant for these calculations because the current
        model evaluates each SKU independently; their stocks are nevertheless
        restored exactly after the diagnostic.
        """
        original_stock = s_ij.copy()
        summary_rows = []
        curve_rows = []

        try:
            for i in P:
                # Start this SKU from zero everywhere.
                for j in L:
                    s_ij[(i, j)] = 0
                ...
                # --------------------------
                depot_values = []
                for s0 in range(max_stock + 1):
                    s_ij[(i, 0)] = s0
                    for j in L:
                        if j != 0:
                            s_ij[(i, j)] = 0

                    mu_test, var_test, theta_test = compute_mu_ij()
                    ebo_test = compute_EBO(mu_test, var_test)
                    tw = total_waiting_time_item(
                        i, mu_test, var_test, theta_test, ebo_test
                    )
                    depot_values.append(tw)

                depot_start, depot_delta, depot_second = _find_discrete_convex_start(
                    depot_values, tol=tol, min_tail_points=min_tail_points
                )
                ...
                summary_rows.append({
                    ...
                    "Location": "Depot (VSM)",
                    "Location index": 0,
                    "Recommended start": depot_start,
                    "Convex in tested range": depot_start is not None,
                    "Tested up to stock": max_stock,
                    "Objective at stock 0": depot_values[0],
                    "Objective at recommended start": (
                        depot_values[depot_start] if depot_start is not None else np.nan
                    ),
                })

                for stock, value in enumerate(depot_values):
                    curve_rows.append({
                        "Item": i,
                        "Location": "Depot (VSM)",
                        "Location index": 0,
                        "Stock": stock,
                        "Total waiting time": value,
                        "Marginal waiting-time reduction": (
                            depot_delta[stock] if stock < len(depot_delta) else np.nan
                        ),
                        "Second difference": (
                            depot_second[stock] if stock < len(depot_second) else np.nan
                        ),
                    })

                # If no convex depot region was found, use 0 only for the base
                # diagnostic and flag the depot result in the summary.
                depot_reference = depot_start if depot_start is not None else 0
                ...
                for base_j in L:
                    ...
                    # Reset this item's bases and use the depot reference found above.
                    s_ij[(i, 0)] = depot_reference
                    for j in L:
                        if j != 0:
                            s_ij[(i, j)] = 0
                    ...
                    for S in range(max_stock + 1):
                        ...
                        base_values.append(tw)

                    base_start, base_delta, base_second = _find_discrete_convex_start(
                        base_values, tol=tol, min_tail_points=min_tail_points
                    )
                    ...
                    summary_rows.append({
                        ...
                        "Location": {
                            ...
                            3: "Regional hub (UK)",
                            4: "Regional hub (UAE)",
                        }.get(base_j, f"Base {base_j}"),
                        "Location index": base_j,
                        "Recommended start": base_start,
                        "Convex in tested range": base_start is not None,
                        "Tested up to stock": max_stock,
                        "Objective at stock 0": base_values[0],
                        "Objective at recommended start": (
                            base_values[base_start] if base_start is not None else np.nan
                        ),
                    })

                    for stock, value in enumerate(base_values):
                        curve_rows.append({
                            "Item": i,
                            "Location": {
                                1: "Virtual hub (NL)",
                            ...
                            "Total waiting time": value,
                            "Marginal waiting-time reduction": (
                                base_delta[stock] if stock < len(base_delta) else np.nan
                            ),
                            "Second difference": (
                                base_second[stock] if stock < len(base_second) else np.nan
                            ),
                        })
            ...
            # Never let the diagnostic alter the optimization starting state.
            s_ij.clear()
            s_ij.update(original_stock)
...
#-------------------------------------------------------------------
    ...
    results = {
        ...
        "convexity_curves": convexity_curves,
    }
